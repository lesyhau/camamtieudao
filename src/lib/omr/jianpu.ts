...
//      现按列投影把宽块切成多个数字格。
import type { Binary, Component, JpNum, Rect, StaffRow, RecognizedScore } from "./types.ts";
import { rright, rbottom, rcx, rcy } from "./types.ts";
import { connectedComponents } from "./ccl.ts";
import type { OcrBackend } from "./ocr.ts";
import { recognizeLyrics } from "./lyrics.ts";
import { recognizeHeader } from "./header.ts";
import { detectSlurs } from "./slur.ts";
