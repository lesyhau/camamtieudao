...
import type { OcrBackend } from "./ocr.ts";
import type { Binary, Rect } from "./types.ts";
import { rright, rbottom } from "./types.ts";
...
function isTauri(): boolean {
  // Always false on the server. The Tauri branch hands tensors to a Rust host process; there
  // is no such host here, and onnxruntime-node is already native so it would buy nothing.
  return false;
}
...
/**
 * onnxruntime-node runs the graph natively and manages its own intra-op threads, so the
 * browser's whole thread negotiation - SharedArrayBuffer, cross-origin isolation, the WebKit
 * worker deadlock, the warmup probe and the single-threaded fallback - has nothing to decide
 * here and is gone.
 */
